...
// If you know the exact JID, uncomment and use this instead
// const TARGET_GROUP_JID = "120363022297061385@g.us"; 
...
async function syncGroupMessages() {
  ...
  // Check if --clear-session flag is provided
  ...
  // Used to track if we need to restart after QR scan
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    // Detect if this is a new pairing that needs restart
    ...
    if (connection === 'open') {
      ...
      try {
        ...
        if (group) {
          ...
          // If no messages were collected, try to manually request chat history
          if (messagesAdded === 0) {
            console.log('No messages added. Attempting to load more...');
            try {
              await sock.sendMessage(targetGroupJid, { text: '.sync' });
              console.log('Sent sync message, waiting 5 more seconds...');
              await new Promise(resolve => setTimeout(resolve, 5000));
            } catch (err) {
              console.log('Error sending sync message:', err);
            }
          }
          ...
          try {
            ...
            // If we need to restart, provide instructions
            if (needsRestart) {
              console.log('\n\x1b[33m==================================\x1b[0m');
              ...
              console.log('\x1b[36mpnpm run sync-group-db\x1b[0m');
              console.log('\x1b[33m==================================\x1b[0m\n');
            }
            ...
            console.error('Error printing sync summary:', err);
            process.exit(1);
          }
        } else {
