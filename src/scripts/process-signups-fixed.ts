...
function parseSignupMessage(message: Message): FormattedSignup | null {
  ...
  // Special handling for common message patterns
  
  // Pattern: "Name1 and Name2 15h" or "Name1 & Name2 15h"
  ...
  // Pattern: "Name in 15h" or "Name 15h"
  ...
  for (const part of parts) {
    ...
    // Extract name from common patterns - allow international characters
    const nameMatch = name.match(/^([A-Za-z\u00C0-\u017F\s.\-']+)(?:\s*\([^)]*\))?/);
    if (nameMatch && nameMatch[1].trim().length > 1) {
      name = nameMatch[1].trim();
      // Only add if it looks like a name (not just a single character or symbol)
      if (name.length > 1 && !/^[\d\W]+$/.test(name)) {
        names.push(name);
      }
    } else if (name.length > 1 && !/^[\d\W]+$/.test(name)) {
      names.push(name);
    }
  }
